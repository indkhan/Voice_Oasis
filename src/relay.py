...
def setup_compliance_logging():
    """Set up detailed compliance logging with file rotation"""
    ...
    log_file = os.path.join(LOGS_DIR, "compliance.log")
    ...
    formatter = logging.Formatter(
        ...
        "%(message)s - metadata: %(extra_metadata)s"
    )
...
compliance_logger = setup_compliance_logging()


# Create a logging adapter to handle metadata
class ComplianceAdapter(logging.LoggerAdapter):
    """Adapter to add metadata to log records"""
    ...
    def process(self, msg, kwargs):
        ...
        # Add metadata to extra dict
        kwargs["extra"]["extra_metadata"] = json.dumps(metadata)

        return msg, kwargs


# Create the adapter instance
compliance = ComplianceAdapter(compliance_logger, {})
...
def log_endpoint(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        metadata = {
            ...
            "chat_session_id": kwargs.get("chat_session_id", "N/A"),
        }

        compliance.info(f"API Request: {method} {endpoint}", metadata=metadata)
    ...
    try:
        ...
        compliance.info(
            ...
            metadata={
                ...
                "speakers_count": len(embeddings_to_save),
            },
        )
        ...
        compliance.info(
            "Memories database saved",
            metadata={
                "file": MEMORIES_FILE,
                "speakers_count": len(memories),
        ...
        compliance.info(
            "Chat sessions database saved",
            metadata={"file": CHAT_SESSIONS_FILE, "sessions_count": len(chat_sessions)},
        )
        ...
        for temp_file in [
            f"{EMBEDDINGS_FILE}.tmp",
            f"{MEMORIES_FILE}.tmp",
            f"{CHAT_SESSIONS_FILE}.tmp",
        ]:
...
def open_session(chat_session_id):
    ...
    if "language" not in body:
        compliance.warning(
            "Session creation failed - missing language parameter",
            metadata={
...
def upload_audio_chunk(chat_session_id, session_id):
    ...
    if session_id not in sessions:
        compliance.warning(
            "Audio chunk upload failed - session not found",
            metadata={"chat_session_id": chat_session_id, "session_id": session_id},
        )
    ...
    if audio_data and len(audio_data) >= 12:
        ...
        if has_riff_header:
            try:
                ...
                voice_activity = detect_voice_activity(audio_float, sample_rate)
                has_voice = np.mean(voice_activity) > 0.2
                ...
                metadata.update(
                    {
                        ...
                        "channels": channels,
                        "has_voice": has_voice,
                        "voice_activity_percentage": float(np.mean(voice_activity)),
                    }
                )
                ...
                if has_voice:
                    ...
                    compliance.info(
                        "Audio chunk processed with voice activity", metadata=metadata
                    )
                else:
                    compliance.info(
                        "Audio chunk skipped - no significant voice activity",
                        metadata=metadata,
                    )
                ...
                compliance.error("Error processing audio chunk", metadata=metadata)
        else:
            compliance.info("Processing raw PCM audio data", metadata=metadata)
...
def close_session(chat_session_id, session_id):
    ...
    if session_id not in sessions:
        compliance.warning(
            "Session close failed - session not found",
            metadata={"chat_session_id": chat_session_id, "session_id": session_id},
        )
        return jsonify({"error": "Session not found"}), 404
    ...
    metadata = {
        "chat_session_id": chat_session_id,
        "session_id": session_id,
        "has_audio": sessions[session_id]["audio_buffer"] is not None,
    }
    ...
    if sessions[session_id]["audio_buffer"] is not None:
        ...
        try:
            ...
            metadata["is_valid_wav"] = is_valid_wav
            metadata["original_audio_size"] = len(audio_buffer)

            if not is_valid_wav:
                compliance.info("Converting raw PCM to WAV format", metadata=metadata)
                try:
                    ...
                    metadata["converted_audio_size"] = len(
                        sessions[session_id]["audio_buffer"]
                    )
                except Exception as e:
                    metadata["conversion_error"] = str(e)
                    compliance.error(
                        "Error converting raw audio to WAV", metadata=metadata
                    )

            # Process speaker identification
            processed_audio = preprocess_audio(sessions[session_id]["audio_buffer"])
            sessions[session_id]["processed_audio"] = processed_audio
            metadata["processed_audio_size"] = len(processed_audio)

            embedding = extract_embedding(processed_audio)
            if embedding is None:
                compliance.error("Failed to extract voice embedding", metadata=metadata)
            ...
            if speaker_id is None:
                ...
                metadata["new_speaker_created"] = True
            ...
            # Analyze sentiment and add to memory
            ...
            if len(updated_memories) > 5:
                ...
                metadata["memories_summarized"] = True

            voice_memories[speaker_id] = updated_memories
            metadata["total_memories"] = len(updated_memories)

            # Extract personal info
            name, order = extract_personal_info(updated_memories)
            if name or order:
                metadata["personal_info"] = {"name": name, "order": order}
            ...
            compliance.info("Session closed successfully", metadata=metadata)

        except Exception as e:
            metadata["error"] = str(e)
            metadata["traceback"] = traceback.format_exc()
            compliance.error("Error processing session close", metadata=metadata)
...
def speech_socket(ws, chat_session_id, session_id):
    ...
    if session_id not in sessions:
        compliance.warning(
            "WebSocket connection failed - session not found", metadata=metadata
        )
    ...
    compliance.info("WebSocket connection established", metadata=metadata)
    ...
    try:
        ...
        while True:
            ...
            metadata["message_received"] = bool(msg)
            compliance.info("WebSocket message received", metadata=metadata)
    except Exception as e:
        metadata["error"] = str(e)
        metadata["traceback"] = traceback.format_exc()
        compliance.error("WebSocket error occurred", metadata=metadata)
    finally:
        compliance.info("WebSocket connection closed", metadata=metadata)
...
def set_memories(chat_session_id):
    ...
    if not chat_history or not isinstance(chat_history, list) or len(chat_history) == 0:
        compliance.warning(
            ...
            metadata={
                "chat_session_id": chat_session_id,
                "received_type": type(chat_history).__name__,
            },
        )
    ...
    if chat_session_id in chat_sessions:
        ...
        if speaker_id and speaker_id in voice_memories:
            ...
            compliance.info("Memories updated successfully", metadata=metadata)
    ...
    compliance.warning(
        "No associated speaker found for chat session", metadata=metadata
    )
...
def get_memories(chat_session_id):
    ...
    if chat_session_id not in chat_sessions:
        compliance.warning("Chat session not found", metadata=metadata)
    ...
    if not speaker_id:
        compliance.info("No speaker identified for chat session", metadata=metadata)
    ...
    if not memories:
        compliance.info("No memories available for speaker", metadata=metadata)
    ...
    compliance.info("Memories retrieved successfully", metadata=metadata)
